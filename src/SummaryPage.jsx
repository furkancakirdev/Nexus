...
export function SummaryPage({ rows, settings, employees, targetRows, annualPool, year, mode, onNavigate }) {
  ...
  const targetMonths = targetRows.filter((row) => row.target > 0);
  ...
  return <main className="page summary-page" id="top">
    <section className="page-heading summary-heading"><div><p className="eyebrow">Yönetim özeti</p><h1>Havuz Genel Bakış</h1><p>{year} performansını, departman hedeflerini ve havuz oluşumunu tek ekranda izleyin.</p></div><span className={`source-badge source-badge--${mode}`}>{mode === "live" ? "CPM canlı" : "Pilot veri"}</span></section>
    <section className="summary-kpis">
      <article><span><IconChartBar /></span><div className="label-value"><small>Net satışlar</small><strong>{money.format(totals.sales)} TL</strong><p>{rows.length} dönem verisi</p></div></article>
      <article><span className="green"><IconWallet /></span><div className="label-value"><small>Net dağıtım havuzu</small><strong>{money.format(annualPool)} TL</strong><p>Hedef bantları, rezerv sonrası</p></div></article>
      <article><span className="blue"><IconUsers /></span><div className="label-value"><small>Uygun personel</small><strong>{eligible} / {distribution.length}</strong><p>{distribution.length - eligible} kişi kapsam dışında</p></div></article>
      <article><span className="amber"><IconTargetArrow /></span><div className="label-value"><small>Hedef tutan departman-ay</small><strong>{metTargetMonths.length} / {targetMonths.length}</strong><p>Temkinli veya büyüme bandı</p></div></article>
    </section>
    ...
    <div className="summary-grid">
      <section className="panel summary-performance"><div className="panel-heading"><div><h2>Aylık Satış ve Kârlılık</h2><p>CPM'den gelen satış ve hesaplanan dağıtıma esas kâr · TL</p></div></div>{chartRows.length ? <div className="summary-chart" role="img" aria-label="Aylık satış ve kârlılık grafiği"><ResponsiveContainer width="100%" height={310}><AreaChart data={chartRows} margin={{ top: 18, right: 18, left: 4, bottom: 4 }}><defs><linearGradient id="salesFill" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="var(--chart-sales)" stopOpacity={.25}/><stop offset="95%" stopColor="var(--chart-sales)" stopOpacity={0}/></linearGradient></defs><CartesianGrid vertical={false} stroke="var(--chart-grid)"/><XAxis dataKey="monthName" tick={{ fontSize: 11, fill: "var(--muted)" }}/><YAxis tickFormatter={(value) => compact.format(value)} tick={{ fontSize: 11, fill: "var(--muted)" }} width={72}/><Tooltip formatter={(value) => `${money.format(value)} TL`}/><Legend iconType="line" wrapperStyle={{ fontSize: 12 }}/><Area dataKey="sales" name="Satış" stroke="var(--chart-sales)" fill="url(#salesFill)" strokeWidth={2}/><Area dataKey="profit" name="Kâr" stroke="var(--chart-profit)" fill="transparent" strokeWidth={2}/></AreaChart></ResponsiveContainer></div> : <div className="report-state report-state--empty">Aylık grafik için veri bulunamadı.</div>}
      <AccessibleChartLegend label="Aylık satış ve kârlılık serileri" items={[{ name: "Satış", color: "var(--chart-sales)" }, { name: "Kâr", color: "var(--chart-profit)" }]} />
      </section>
      <aside className="summary-attention"><div className="summary-attention__head"><IconAlertTriangle/><strong>Dikkat Gerektirenler</strong><span>{reviewPeriods.length + distributionResult.departments.filter((item) => item.allocationStatus === "review-required").length}</span></div><div className="attention-list"><button onClick={() => onNavigate("audit")}><span className="attention-icon warning"><IconFileCheck/></span><div><strong>{reviewPeriods.length} dönem maliyet incelemesi var</strong><small>Eksik satırları CPM Denetim merkezinden doğrulayın.</small></div><IconArrowRight/></button><button onClick={() => onNavigate("goals")}><span className="attention-icon info"><IconTargetArrow/></span><div><strong>{metTargetMonths.length} departman-ay hedefi tuttu</strong><small>Hedef, eşik, bant ve aylık havuzu inceleyin.</small></div><IconArrowRight/></button><button onClick={() => onNavigate("settings")}><span className="attention-icon ok"><IconUsers/></span><div><strong>{distribution.length} personel parametresi</strong><small>Katsayı, eşit dağıtım ve sabit pay kurallarını yönetin.</small></div><IconArrowRight/></button></div></aside>
    </div>
    <section className="summary-shortcuts"><button onClick={() => onNavigate("sales")}><IconChartBar/><span><strong>Satış analizini aç</strong><small>Aylık satış, iskonto ve kârlılık</small></span><IconArrowRight/></button><button onClick={() => onNavigate("goals")}><IconTargetArrow/><span><strong>Hedefleri incele</strong><small>Departman hedefi, bandı ve havuzu</small></span><IconArrowRight/></button><button onClick={() => onNavigate("reports")}><IconFileCheck/><span><strong>Yönetim raporu</strong><small>Excel ve PDF çıktıları</small></span><IconArrowRight/></button></section>
  </main>;
}
