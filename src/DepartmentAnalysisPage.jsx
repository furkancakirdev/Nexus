...
export function DepartmentAnalysisPage({ year, mode: appMode, consolidatedRows = [], minimumCoverage = 80, externalRefreshToken = 0, eurRateSets = {} }) {
  ...
  const productRanking = data.topProductsByDepartment?.[department]
    || (department === "all" ? data.topProducts : []);
  ...
  return <main className="page department-page" id="top">
    ...
    {tab === "rankings" && <section className="ranking-grid">
      ...
      <article className="panel ranking-panel ranking-panel--evidence"><div className="panel-heading"><div><h2>Kanıt adayı sorumlular</h2><p>Kaynakta bulunan ancak resmi performans sıralamasına dahil değildir</p></div><IconAlertTriangle size={21} /></div><div className="ranking-list">{(data.ownerEvidenceTotals || []).filter((item) => !isOfficialOwnerRankingCandidate(item) && (department === "all" || item.department === department)).map((item) => { const eur = projectDepartmentEurMetric(item); return <div key={item.id} className="ranking-row--review" role="button" tabIndex="0" onClick={() => { setTab("ledger"); setSearch(item.code || item.name); }} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setTab("ledger"); setSearch(item.code || item.name); } }} title={`${item.name} belgelerini incele`}><span className="rank">—</span><span className="ranking-name"><strong title={item.name} aria-label={item.name}>{item.name}</strong><small>{item.code || "—"} · {item.location || "—"} · {actorActivityLabel(item.active)}</small></span><DepartmentBadge department={item.department} /><span className="ranking-value"><strong>{formatEur(eur.netSales)}</strong><small>İnceleme gerekli · resmi performansa dahil değil</small></span></div>; })}{!(data.ownerEvidenceTotals || []).some((item) => !isOfficialOwnerRankingCandidate(item)) && <p className="empty-copy">Resmi sıralama dışında kalan kanıt adayı bulunamadı.</p>}</div></article>
      ...
      <article className="panel ranking-panel"><div className="panel-heading"><div><h2>{department === "service" ? "Servis Gelir Kalemleri" : department === "parts" ? "Parça Ürün Yoğunlaşması" : "Departman Ürün Yoğunlaşması"}</h2><p>Seçili departmanın kanonik ledger net satış ve marj görünümü</p></div><IconPackage size={21} /></div><div className="ranking-list">{(productRanking || []).map((item, index) => { const eur = projectDepartmentEurMetric(item); return <div key={item.id}><span className="rank">{index + 1}</span><span className="ranking-name"><strong>{item.name}</strong><small>{item.code}{item.brand && item.brand !== "—" ? ` · ${item.brand}` : ""}</small></span><span className="ranking-value"><strong>{formatEur(eur.netSales)}</strong><small>Kâr {formatEur(eur.profit)} · Marj {percent(eur.margin)}</small></span></div>; })}{!productRanking?.length && <p className="empty-copy">Departman ürün verisi henüz yok.</p>}</div></article>
    </section>}
