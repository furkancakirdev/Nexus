...
export function DepartmentAnalysisPage({ year, mode: appMode, consolidatedRows = [], minimumCoverage = 80, externalRefreshToken = 0, eurRateSets = {} }) {
  ...
  return <main className="page department-page" id="top">
    ...
    {tab === "rankings" && <section className="ranking-grid">
      ...
      <article className="panel ranking-panel"><div className="panel-heading"><div><h2>Kanıt adayı sorumlular</h2><p>Ticari sahiplik kanıtı tamamlanmamış; resmi performans sıralamasına dahil değildir.</p></div><IconInfoCircle size={21} /></div><div className="ranking-list">{(data.ownerEvidenceTotals || []).filter((item) => !isOfficialOwnerRankingCandidate(item) && (department === "all" || item.department === department)).map((item) => { const eur = projectDepartmentEurMetric(item); return <div key={item.id} style={{ cursor: "pointer" }} onClick={() => { setTab("ledger"); setSearch(item.code || item.name); }} title={`${item.name} belgelerini incele`}><span className="rank">•</span><span className="ranking-name"><strong title={item.name} aria-label={item.name}>{item.name}</strong><small>{item.code || "—"} · İnceleme gerekli</small></span><DepartmentBadge department={item.department} /><span className="ranking-value"><strong>{formatEur(eur.netSales)}</strong><small>{money.format(item.documentCount)} belge · kanıt adayı</small></span></div>; })}{!data.ownerEvidenceTotals?.length && <p className="empty-copy">İnceleme bekleyen sorumlu adayı yok.</p>}</div></article>
