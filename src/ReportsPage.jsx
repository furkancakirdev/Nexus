...
const tabs=[
  ["summary","Yönetim Özeti"],["brand","Marka"],["dealer","Bayi"],["channel","Kanal / Modül"],["service","Teknik Servis"],["cost","Alım ve Maliyet"],["discount","İskonto ve İade"],
];
...
export function ReportsPage({settings,employees,targetRows,annualPool,year,rows,eurRateSets={},canonicalMetric=null}){
  ...
  const brand=toEurRows(projections.brand); const dealer=toEurRows(projections.dealer); const channel=toEurRows(projections.channel); const service=toEurRows(projections.service); const costs=toEurRows(projections.cost);
  ...
  const datasets={brand,dealer,channel,service,cost:costs,pool};
  ...
  <section className="report-kpis"><article><span><IconChartBar/></span><div><small>Net satış · EUR</small><strong>{eurNetSales == null ? "—" : eurFormat.format(eurNetSales)}</strong><p>{eurNetSales == null ? "Kur kanıtı bekleniyor" : eurRateSourceLabel}</p></div></article><article><span className="report-kpi--green"><IconWallet/></span><div><small>Hesaplanan kâr · EUR</small><strong>{eurProfit == null ? "—" : eurFormat.format(eurProfit)}</strong><p>{eurComplete ? "Doğrulanmış WAC ve kur seti karşılığı" : "Maliyet kanıtı bekleniyor"}</p></div></article><article><span className="report-kpi--blue"><IconBuilding/></span><div><small>Bayi net satışı · EUR</small><strong>{projections.summary?.dealerEurNetSales == null ? "—" : eurFormat.format(projections.summary.dealerEurNetSales)}</strong><p>EUR dönüşüm kanıtı</p></div></article><article><span className="report-kpi--amber"><IconDatabase/></span><div><small>Teknik servis net satışı · EUR</small><strong>{projections.summary?.serviceEurNetSales == null ? "—" : eurFormat.format(projections.summary.serviceEurNetSales)}</strong><p>EUR dönüşüm kanıtı</p></div></article><article><span className="report-kpi--amber"><IconAlertTriangle/></span><div><small>İnceleme tutarı · kaynak TRY</small><strong>{reviewNetSales == null ? "—" : `${money.format(reviewNetSales)} TL`}</strong><p>{reviewNetSales == null ? "Maliyet kanıtı bekleniyor" : "Gerçek kâr ve marj kapsamı dışında"}</p></div></article></section>
  <section className="panel" aria-label="Rapor görünümü"><div className="report-tabs" role="tablist" aria-label="Rapor bölümleri">{tabs.map(([id,label])=><button key={id} type="button" role="tab" aria-selected={active===id} className={active===id?"active":""} onClick={()=>setActive(id)}>{label}</button>)}</div></section>
  {active === "summary" && <>
    <section className="panel report-finance-evidence">
      ...
      <div className="table-scroll"><table className="report-table"><thead><tr><th>Kaynak dövizi</th><th>Net satış · kaynak</th><th>Maliyet · kaynak</th><th>Brüt kâr · kaynak</th></tr></thead><tbody>
        {currencyBaskets.map(([currency, basket])=><tr key={currency}><th>{currency}</th><td>{money.format(basket.netSales)} {currency}</td><td>{money.format(basket.cost)} {currency}</td><td className={basket.profit >= 0 ? "positive" : "negative"}>{money.format(basket.profit)} {currency}</td></tr>)}
        {!currencyBaskets.length&&<tr><td colSpan="4" className="empty-state">Finansal kanıt henüz bulunamadı.</td></tr>}
      </tbody></table></div>
      <p className="table-note">İnceleme gereken tutar: {reviewNetSales == null ? "—" : `${money.format(reviewNetSales)} TL kaynak tutarı`}. Bu tutar resmî kâr veya havuz toplamına dahil edilmez.</p>
    </section>
    <div className="report-grid">
      ...
      <aside className="report-notes"><div className="report-notes__head"><IconReportAnalytics/><strong>Yönetim Özeti</strong></div><ul><li>DBS kodlu bayiler {projections.summary?.dealerEurNetSales == null ? "—" : eurFormat.format(projections.summary.dealerEurNetSales)} EUR net satış yarattı.</li><li>Teknik servis kaynaklı satış {projections.summary?.serviceEurNetSales == null ? "—" : eurFormat.format(projections.summary.serviceEurNetSales)} EUR.</li><li>İnceleme kapsamı finansal kâr ve marj toplamlarına dahil edilmez.</li></ul></aside>
    </div>
  </>}
  ...
  {active !== "summary" && active !== "discount" && <section className="panel distribution-report"><div className="distribution-report__head"><div><h2>{tabs.find(([id])=>id===active)?.[1]}</h2><p>{active === "pool" ? "Bağımsız EUR havuz kanıtı bekleniyor; kaynak TRY resmi EUR yerine gösterilmez" : "EUR karşılığı"}</p></div><label className="goal-search"><IconSearch size={17}/><input value={query} onChange={(event)=>setQuery(event.target.value)} placeholder="Ara"/></label></div><div className="table-scroll"><table className="report-table"><thead><tr><th>Boyut</th><th>Net satış · EUR</th><th>Maliyet · EUR</th><th>Kâr · EUR</th><th>Marj</th></tr></thead><tbody>{filtered.map((item)=><tr key={item.name}><th>{item.name}</th><td>{item.netSales == null ? "—" : eurFormat.format(item.netSales)}</td><td>{item.cost == null ? "—" : eurFormat.format(item.cost)}</td><td className={item.profit == null ? "" : item.profit >= 0 ? "positive" : "negative"}>{item.profit == null ? "—" : eurFormat.format(item.profit)}</td><td>%{item.margin === null || item.margin === undefined ? "—" : Number(item.margin).toFixed(1)}</td></tr>)}{!filtered.length&&<tr><td colSpan="5" className="empty-state">Veri bulunamadı.</td></tr>}</tbody></table></div></section>}
