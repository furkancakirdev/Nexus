...
export function ReportsPage({settings,employees,targetRows,annualPool,year,rows,eurRateSets={},canonicalMetric=null}){
  ...
  <section className="report-kpis"><article><span><IconChartBar/></span><div><small>Net satış · EUR</small><strong>{eurNetSales ? `${money.format(eurNetSales)} EUR` : "—"}</strong><p>{eurRateCount ? `${eurRateSourceLabel} · ${eurRateCount} dönem` : "Kur kanıtı bekleniyor"}</p></div></article><article><span className="report-kpi--green"><IconWallet/></span><div><small>Hesaplanan kâr · EUR</small><strong>{eurProfit ? eurFormat.format(eurProfit) : "—"}</strong><p>{eurRateCount ? "Doğrulanmış kur seti karşılığı" : "Kur kanıtı bekleniyor"}</p></div></article><article><span className="report-kpi--blue"><IconBuilding/></span><div><small>Bayi satışı · KDV hariç</small><strong>{money.format(projections.summary?.dealerNetSales)} TL</strong><p>DBS kodlu {dealer.length} bayi</p></div></article><article><span className="report-kpi--amber"><IconDatabase/></span><div><small>Teknik servis · KDV hariç</small><strong>{money.format(projections.summary?.serviceNetSales)} TL</strong><p>Kaynak evrak 64</p></div></article><article><span className="report-kpi--amber"><IconAlertTriangle/></span><div><small>İnceleme gerekli · KDV hariç</small><strong>{reviewNetSales == null ? "—" : `${money.format(reviewNetSales)} TL`}</strong><p>{reviewLines} satırda resmî kâr yayınlanmaz</p></div></article></section>
  ...
  {active==="summary"?<><section className="panel report-finance-evidence"><div className="panel-heading"><div><h2>Finansal kanıt özeti</h2><p>Net satış, maliyet ve kâr kaynak dövizinde ayrı tutulur; EUR yalnız tam kur kanıtında yayınlanır.</p></div><span className={`evidence-state ${eurComplete ? "" : "evidence-state--review"}`}>{eurComplete ? "EUR tamamlandı" : "EUR inceleme gerekli"}</span></div><div className="table-scroll"><table className="report-table"><thead><tr><th>Kaynak dövizi</th><th>Satır</th><th>Net satış · KDV hariç</th><th>Maliyet · KDV hariç</th><th>Brüt kâr · KDV hariç</th></tr></thead><tbody>{currencyBaskets.map(([currency, basket])=><tr key={currency}><th>{currency}</th><td>{money.format(basket.lineCount)}</td><td>{money.format(basket.netSales)} {currency}</td><td>{money.format(basket.cost)} {currency}</td><td className={basket.profit >= 0 ? "positive" : "negative"}>{money.format(basket.profit)} {currency}</td></tr>)}{!currencyBaskets.length&&<tr><td colSpan="5" className="empty-state">Finansal kanıt henüz bulunamadı.</td></tr>}</tbody></table></div><p className="table-note">İnceleme gereken tutar: {reviewNetSales == null ? "—" : `${money.format(reviewNetSales)} TL`} · {reviewLines} satır. Bu tutar resmî kâr veya havuz toplamına dahil edilmez.</p></section><div className="report-grid"><section className="panel department-report"><div className="panel-heading"><div><h2>En Yüksek Net Satışlı Markalar</h2><p>CPM stok kartı marka alanı · KDV hariç</p></div></div>{!loading&&!error&&!brand.length ? <div className="report-state report-state--empty">Marka grafiği için veri bulunamadı.</div> : !loading&&!error && <div className="report-chart" role="img" aria-label="Marka satış ve kâr grafiği"><ResponsiveContainer width="100%" height={320}><BarChart data={brand.slice(0,10)}><CartesianGrid vertical={false} stroke="var(--chart-grid)"/><XAxis dataKey="name" tick={{fontSize:10,fill:"var(--muted)"}}/><YAxis tickFormatter={(v)=>`${Math.round(v/1e6)} Mn`} tick={{fill:"var(--muted)"}}/><Tooltip formatter={(v)=>`${money.format(v)} TL`}/><Legend iconType="square" wrapperStyle={{fontSize:12}}/><Bar dataKey="netSales" name="Net satış" fill={REPORT_CHART_COLORS.sales} radius={[3,3,0,0]}/><Bar dataKey="profit" name="Kâr" fill={REPORT_CHART_COLORS.profit} radius={[3,3,0,0]}/></BarChart></ResponsiveContainer></div>}</section><aside className="report-notes"><div className="report-notes__head"><IconReportAnalytics/><strong>Yönetim Özeti</strong></div><ul><li>{brand.length} marka satış hareketi oluşturdu.</li><li>DBS kodlu bayiler {money.format(projections.summary?.dealerNetSales)} TL net satış yarattı.</li><li>Teknik servis kaynaklı satış {money.format(projections.summary?.serviceNetSales)} TL.</li><li>{confidence.find(r=>r.name==="İnceleme gerekli")?.lines||0} satır maliyet incelemesi bekliyor.</li></ul></aside></div></>:active==="discount"?<section className="panel distribution-report"><div className="panel-heading"><div><h2>İskonto ve İade Özeti</h2><p>Brüt satıştan düşülen ticari hareketler</p></div></div><div className="report-kpis"><article><span><IconDiscount/></span><div><small>İskonto</small><strong>{money.format(totals.discounts)} TL</strong></div></article><article><span><IconChartBar/></span><div><small>İade</small><strong>{money.format(totals.returns)} TL</strong></div></article></div></section>:<section className="panel distribution-report"><div className="distribution-report__head"><div><h2>{tabs.find(([id])=>id===active)?.[1]}</h2><p>{filtered.length} analiz satırı</p></div><label className="goal-search"><IconSearch size={17}/><input value={query} onChange={(event)=>setQuery(event.target.value)} placeholder="Ara"/></label></div><div className="table-scroll"><table className="report-table"><thead><tr><th>Boyut</th><th>Satır</th><th>{active==="pool"?"Toplam havuz":"Net satış / pay"}</th><th>{active==="pool"?"Dağıtılamayan":"Maliyet"}</th><th>{active==="pool"?"Dağıtılan":"Kâr"}</th><th>{active==="pool"?"Dağıtım oranı":"Marj"}</th></tr></thead><tbody>{filtered.map((item)=><tr key={item.name}><th>{item.name}</th><td>{money.format(item.lines)}</td><td>{money.format(item.netSales)} TL</td><td>{money.format(item.cost)} TL</td><td className={item.profit>=0?"positive":"negative"}>{money.format(item.profit)} TL</td><td>%{item.margin === null ? "—" : Number(item.margin).toFixed(1)}</td></tr>)}{!filtered.length&&<tr><td colSpan="6" className="empty-state">Veri bulunamadı.</td></tr>}</tbody></table></div></section>}
